...
Texture2D _crossOverMask;
uniform float4 _crossOverMask_ST;
Texture2D _crossOverAlbedo;
uniform float4 _crossOverAlbedo_ST;
...
Texture2D _SSSThicknessMask;
uniform float4 _SSSThicknessMask_ST;

Texture2D _Set_RimLightMask;
uniform float4 _Set_RimLightMask_ST;

Texture2D _MatCapTexAdd;
uniform float4 _MatCapTexAdd_ST;
Texture2D _MatCapTexMult;
uniform float4 _MatCapTexMult_ST;
Texture2D _MatCapTexEmis;
uniform float4 _MatCapTexEmis_ST;
Texture2D _MatCapTexHardMult;
uniform float4 _MatCapTexHardMult_ST;
...
uniform float4 _EmissionColorTex_ST;
Texture2D _EmissionColorTex2;
uniform float4 _EmissionColorTex2_ST;

TextureCube _CubemapFallback;
uniform float4 _CubemapFallback_HDR;
...
Texture2D _NormalMapDetail;
uniform float4 _NormalMapDetail_ST;
Texture2D _DetailNormalMask;
uniform float4 _DetailNormalMask_ST;

Texture2D _DynamicShadowMask;
uniform float4 _DynamicShadowMask_ST;
uniform float4 _DynamicShadowMask_TexelSize;
...
g2f vert(appdata v)
{
    ...
    //v.vertex = ClipToObjectPos(o.pos);
...
) : SV_TARGET
{
    ...
    float3 normalMap = UnpackNormal(
        _NormalMap.SampleGrad(sampler_NormalMap, uv_normalMap.uv, uv_normalMap.dx, uv_normalMap.dy));
    if (_DetailNormalMapScale01) //// slider > 0
    {
        float4 normalDetailMask = _DetailNormalMask.SampleGrad(sampler_MainTex, uv_normalMapDetailMask.uv,
                                                               uv_normalMapDetailMask.dx, uv_normalMapDetailMask.dy);
        float3 normalMapDetail = UnpackNormal(
            _NormalMapDetail.SampleGrad(sampler_NormalMap, uv_normalMapDetail.uv, uv_normalMapDetail.dx,
                                        uv_normalMapDetail.dy));
        normalMap = lerp(normalMap, BlendNormals(normalMap, normalMapDetail),
                         (normalDetailMask.g * _DetailNormalMapScale01));
    }
    ...
    half4 detailMap = SetupDetail(
        _DetailMap.SampleGrad(sampler_DetailMap, uv_detalAlbedo.uv, uv_detalAlbedo.dx, uv_detalAlbedo.dy));
    ...
    #ifndef NotAlpha
    float4 clipMask = _ClippingMask.Sample(sampler_ClippingMask,
                                           TRANSFORM_TEX(UVPick01(i.uv01, i.uv02), _ClippingMask));
    ...
    if (_useDiffuseAlbedoTexturesSet) //// _useDiffuseAlbedoTexturesSet
    {
        ...
        {
        default:
        case 0: shadeAlbedoTex2 = coreAlbedoTex;
            break;
        case 1: shadeAlbedoTex2 = mainTex;
            break;
        case 2: shadeAlbedoTex2 = backwardAlbedoTex;
            break;
        }
        UNITY_BRANCH
        ...
        {
        default:
        case 0: shadeAlbedoTex3 = backwardAlbedoTex;
            break;
        case 1: shadeAlbedoTex3 = coreAlbedoTex;
            break;
        case 2: shadeAlbedoTex3 = mainTex;
            break;
        }
    }
    ...
    // [1] = camera y upward, [2] = camera z forward
    ...
    */
        ...
        if (isAmbientOnlyMap)
        /// lightAtten is undefined in scenes without directional lights. Using this raw is unstable so we correct when missing the shadow data.
        {
            lightAtten = 1;
        }
    ...
    shadowAtten = RemapRange(shadowAtten, max(0, _LightShadowData.x - .001), 1, 0, 1);
    //// floor shadow to 0.0, to normalize
    ...
    shadowAtten = saturate(RemapRange(shadowAtten + shadowRemoval, 0, 1, _LightShadowData.x + .001, 1));
    //// then return 0.0 to floor

    #if defined(SHADOWS_SCREEN) && defined(UNITY_PASS_FORWARDBASE) // fix screen space shadow arficats from msaa
    shadowAtten = SSDirectionalShadowAA(i._ShadowCoord, shadowAtten);
    #endif
    ...
    half3 refGIcol = shadeSH9LinearAndWhole(float4(normalize(i.wNormal + dEnv.dirViewReflection), 1));
    ...
    float shadowTex_1 = _Set_1st_ShadePosition.SampleGrad(sampler_Set_1st_ShadePosition, uv_ShadePosition.uv,
                                                          uv_ShadePosition.dx, uv_ShadePosition.dy).g;
    float shadowTex_2 = _Set_2nd_ShadePosition.SampleGrad(sampler_Set_1st_ShadePosition, uv_ShadePosition.uv,
                                                          uv_ShadePosition.dx, uv_ShadePosition.dy).g;
    //// Assist for shadow mask
    float shadeRamp_n1 = dDiff.ndl; //// ndl Core area
    float shadeRamp_n2 = dDiff.ndl;
    //// ndl Backward area
    ...
    if (_UseLightMap)
    {
        ...
        lightMask.g = saturate(RemapRange01(lightMask.g, _lightMap_remapArr[2], _lightMap_remapArr[3]));
        ////[0,1]->[A,B]->clamp
        ...
        float2 AOmCalibrate_1 = (toonOffsetMask_1.xx * float2(_toonLambAry_01[0], _toonLambAry_01[0]) + float2(
            _toonLambAry_01[1], _toonLambAry_01[1]));
        ...
        float2 AOmCalibrate_2 = (toonOffsetMask_2.xx * float2(_toonLambAry_02[0], _toonLambAry_02[0]) + float2(
            _toonLambAry_02[1], _toonLambAry_02[1]));
    ...
    //// Diffusion. Ramp shading and surface albedo and light albedo mixer
                    ...
                    //// Normalized values:
    UNITY_BRANCH
    if (_ToonRampLightSourceType_Backwards)
    //// diffuse lighting: backface area is part of shadow thus indirect light only
    {
    ...
    if (_useCrossOverRim)
    {
        ...
        half dbCrossoverDot = crossOverMask * (pow(
            StepFeatherRemap(.5 - dDiff.ndv * .5, _crossOverStep, _crossOverFeather),
            exp2(lerp(3, 0, _crosspOverRimPow))));
        ...
        case 0:
            ...
            break;
        case 1: bd_albedoTex = shadeAlbedoTex1.rgb;
            break;
        case 2: bd_albedoTex = coreAlbedoTex.rgb;
            break;
        case 3: bd_albedoTex = backwardAlbedoTex.rgb;
            break;
        }
    ...
    if (_DetailAlbedo)
    {
        shadeColor_albedo = lerp(sqrt(shadeColor_albedo), (detailMap.r < 0.0) ? 0.0 : 1.0,
                                 abs(detailMap.r) * detailMask.r * _DetailAlbedo);
    ...
    float4 specularMask = _Set_HighColorMask.SampleGrad(sampler_MainTex, uv_specularMask.uv, uv_specularMask.dx,
                                                        uv_specularMask.dy);
    ...
    {
        smoothness = lerp(smoothness, (detailMap.b < 0.0) ? 0.0 : 1.0,
                          abs(detailMap.b) * detailMask.b * _DetailSmoothness);
    }
    ...
    {
        half3 tempCol = lerp(1, shadeColor_albedo, _highColTexSource);
        //// want 1.0 mix for countering extreme dark 0.0 RGBs
        half3 AlbedoHSVI = HSVI_controller(tempCol, _SpecularMaskHSV.x, _SpecularMaskHSV.y, _SpecularMaskHSV.z,
                                           _SpecularMaskHSV.w);
    ...
    {
        diffuseScale = EnergyConservationBetweenDiffuseAndSpecular_ac(_UseDiffuseEnergyConservation, specularSrcCol,
                                                                      oneMinusReflectivity);
    }
    else if (_WorkflowMode == 1) //// metallic
    {
        diffuseScale = DiffuseAndSpecularFromMetallic_ac(_UseDiffuseEnergyConservation, specOverrideColor, mg2[0],
                                                         specularSrcCol, oneMinusReflectivity);
    ...
    {
        float envSmoothness = saturate(1 - perceptualRoughnessGloss + (2 * _envRoughness - 1));
        //// wrong name (its smoothness)
    ...
    if (_MatCap)
    {
        ...
        if (_CameraRolling_Stabilizer)
        {
            ...
            //// get world upward (camera matrix is world)
        ...
        //// normalmap rotate
        float2 rot_MatCapNmUV = rotateUV(UVPick01(i.uv01, i.uv02, _uvSet_NormalMapForMatCap), float2(0.5, 0.5),
                                         (_Rotate_NormalMapForMatCapUV * 3.141592654));
        //// normal map
        UV_DD uv_matcap_nm = UVDD(TRANSFORM_TEX(rot_MatCapNmUV, _NormalMapForMatCap));
        float4 normalMapForMatCap = _NormalMapForMatCap.SampleGrad(sampler_NormalMap, uv_matcap_nm.uv, uv_matcap_nm.dx,
                                                                   uv_matcap_nm.dy);
        ...
        float3 normalBlendMatcapUVBase = (mul(UNITY_MATRIX_V, float4(dirView, 0)).xyz * float3(-1, -1, 1)) +
            float3(0, 0, 1);
        float3 noSknewViewNormal = (normalBlendMatcapUVBase * dot(normalBlendMatcapUVBase, normalBlendMatcapUVDetail) /
            normalBlendMatcapUVBase.z) - normalBlendMatcapUVDetail;
        ...
        float2 rot_MatCapUV = rotateUV(scl_MatCapUV, float2(0.5, 0.5),
                                       (_Rotate_MatCapUV * 3.141592654) + matcapRotStablizer);
        //// get blur
        ...
        float4 matCapTexAdd = _MatCapTexAdd.SampleLevel(sampler_MatCap_Trilinear_clamp, matcapUV,
                                                        perceptualRoughnessToMipmapLevel_ac(mcBlur0, mcLodMax0));
        float4 matCapTexMult = _MatCapTexMult.SampleLevel(sampler_MatCap_Trilinear_clamp, matcapUV,
                                                          perceptualRoughnessToMipmapLevel_ac(mcBlur1, mcLodMax1));
        float4 matCapTexEmis = _MatCapTexEmis.SampleLevel(sampler_MatCap_Trilinear_clamp, matcapUV,
                                                          perceptualRoughnessToMipmapLevel_ac(mcBlur2, mcLodMax2));
        float4 matCapTexHardMult = _MatCapTexHardMult.SampleLevel(sampler_MatCap_Trilinear_clamp, matcapUV,
                                                                  perceptualRoughnessToMipmapLevel_ac(
                                                                      mcBlur3, mcLodMax3));
        ////
    ...
    {
        float envLOD;
        float3 reflDir0 = BoxProjection(dEnv.dirViewReflection, i.worldPos, unity_SpecCube0_ProbePosition,
        ...
        if (_CubemapFallbackMode < 2) //// not override cubemap. solve real cubemaps
        {
            ...
            if (unity_SpecCube0_BoxMin.w < 0.9999) //// 2nd blend cubemap
            {
                ...
                envLOD = perceptualRoughnessToMipmapLevel_ac(roughnessReflection, lodMax);
                float3 reflDir1 = BoxProjection(dEnv.dirViewReflection, i.worldPos, unity_SpecCube1_ProbePosition,
    ...
    {
        half4 rimLightMaskTex = _Set_RimLightMask.SampleGrad(sampler_MainTex, uv_rimLight.uv, uv_rimLight.dx,
                                                             uv_rimLight.dy);
        ...
        if (_rimAlbedoMix)
        {
            ...
            {
            default:
            case 0: rimTexAlbedo = shadeColor_albedo;
                break;
            case 1: rimTexAlbedo = specularSrcCol;
                break;
            }
        ...
        {
            half rimEdgeBoundary1 = saturate(RemapRange(rimLightPower1, _rimLightOverToneLow, _rimLightOverToneHigh, 0,
                                                        1)); //// edge color
            half rimEdgeBoundary2 = saturate(RemapRange(rimLightPower2, _rimLightOverToneLow, _rimLightOverToneHigh, 0,
                                                        1)); //// edge color
    ...
    float4 emissionTex1 = _Emissive_Tex.SampleGrad(sampler_Emissive_Tex, uv_EmissionCol_1.uv, uv_EmissionCol_1.dx,
                                                   uv_EmissionCol_1.dy);
    float4 emissionTex2 = _EmissionColorTex2.SampleGrad(sampler_EmissionColorTex2, uv_EmissionCol_2.uv,
                                                        uv_EmissionCol_2.dx, uv_EmissionCol_2.dy);
    ...
    UNITY_BRANCH if (_emissionMixTintDiffuseSlider)
    {
        ...
        if (_emissionMixTintDiffuseSlider)
        {
            ...
            {
            default:
            case 0: diffuseTintAlbedo = shadeAlbedoTex1.rgb;
                break;
            case 1: diffuseTintAlbedo = shadeAlbedoTex2.rgb;
                break;
            case 2: diffuseTintAlbedo = shadeAlbedoTex3.rgb;
                break;
            }
        }
        emissionTintChain = lerp(emissionTintChain, emissionTintChain * diffuseTintAlbedo,
                                 _emissionMixTintDiffuseSlider);
    }
    UNITY_BRANCH
    if (_MatCap)
    {
        //// matcap emission
        half mcMask;
        if (_matcapEmissMaskSwitch == 1) { mcMask = matcapMask.a; }
        else { mcMask = 1; }
    ...
    {
        //// use mask
        UV_DD uv_emissiveMask = UVDD(
            TRANSFORM_TEX(UVPick01(i.uv01, i.uv02, _uvSet_EmissionColorTex), _EmissionColorTex));
        emissiveMask = _EmissionColorTex.SampleGrad(sampler_EmissionColorTex, uv_emissiveMask.uv, uv_emissiveMask.dx,
                                                    uv_emissiveMask.dy).g;
    ...
    if (_MatCap) //// mix diffuse matcap
    {
        ...
        shadeColor += (diffMixMC * lightSimpleSystem * mcMixMult.rgb * _MatCapColMult.rgb * _MatCapColMult.a *
            matcapMask.r);
    }
    ...
    if (true) //// fake function incapsulation
    {
        ...
        colFernelLight = lerp(LinearRgbToLuminance_ac(colFernelLight), colFernelLight, _envOnRimColorize);
        //// cubemap color
    ...
    if (_ENVMmode > 0) //// using env
    {
        ...
        float envGrazeMask = max((envRimMask), ((envGrazeRimMix_) ? max(rimLightMask1, rimLightMask2) : 0));
        //// mix rim types
        ...
        //// light source hybrid of reflection and indirect: [0,1] * [1,n]
    ...
    if (_MatCap)
    {
        ...
        half mcMask;
        if (_matcapSpecMaskSwitch == 1) { mcMask = matcapMask.b; }
        else { mcMask = aoSpecularM; }
    ...
    if (_forceLightClamp)
    {
        ...
        //// grab all light at max potential
        if (sceneIntensity > 1.0) //// bloom defaults at > 1.1
        {
            half3 correctedReflect = colReflect * alpha;
            //// correction for skybox intensity beign affected. This is refactoring reflection (and if premultiply alpha was used)
    ...
    float NdotL = (dot(Unity_SafeNormalize(UnityWorldSpaceLightDir(i.worldPos)), dirNormal)); // Calculate NdotL

    // Generic lighting for effects with shadow applied.
    float3 effectLightShadow = _LightColor0.rgb * max((1 + NdotL) * shadowAtten, 0);
    #if defined(UNITY_PASS_FORWARDBASE)
    effectLightShadow += GetSHAverage();
    #endif

    addEmissive(emissionMix, i, effectLightShadow, fragColor);
    #else
