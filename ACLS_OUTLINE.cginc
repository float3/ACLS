...
uniform Texture2D _OutlineTex;
uniform float4 _OutlineTex_ST;

uniform Texture2D _outlineEmissionTint;
uniform float4 _outlineEmissionTint_ST;
uniform Texture2D _outlineEmissionMask;
uniform float4 _outlineEmissionMask_ST;
...
#ifndef NotAlpha
uniform Texture2D _ClippingMask;
...
v2f vert(appdata v)
{
    ...
    float outlineControlTex = tex2Dlod(_Outline_Sampler,
                                       float4(TRANSFORM_TEX(UVPick01(o.uv01, o.uv02), _Outline_Sampler), 0, 0)).r;
    ...
    bool frontFace : SV_IsFrontFace) : SV_Target
{
    ...
    alpha = (alpha * sampleCount) + 1. - dither;
    //// define exact floats, not doing so miss-interrupts (uint) conversion.
    ...
    //// emission
    UV_DD uv_EmissionCol_1 = UVDD(
        TRANSFORM_TEX(UVPick01(i.uv01, i.uv02, _uvSet_EmissionColorTex), _outlineEmissionTint));
    float4 emissionTex1 = _outlineEmissionTint.SampleGrad(sampler_outlineEmissionTint, uv_EmissionCol_1.uv,
                                                          uv_EmissionCol_1.dx, uv_EmissionCol_1.dy);
    ...
    {
        UV_DD uv_EmissionMask = UVDD(TRANSFORM_TEX(UVPick01(i.uv01, i.uv02, _uvSet_EmissionColorTex),
                                                   _outlineEmissionMask));
        half emissiveMask = _outlineEmissionMask.SampleGrad(sampler_outlineEmissionMask, uv_EmissionMask.uv,
                                                            uv_EmissionMask.dx, uv_EmissionMask.dy).g;
    ...
    if (_forceLightClamp)
    {
        ...
        //// grab all light at max potencial
