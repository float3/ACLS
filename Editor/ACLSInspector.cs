    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            if (showBasics)
            {
                ...
                materialEditor.TexturePropertySingleLine(new GUIContent("Main Texture (Ramp Forward Source)", "Main texture. As Forward Area intended for surface most towards light and the visual effect of being in direct light."), _MainTex, _Color);
                ...
                materialEditor.TexturePropertySingleLine(new GUIContent("Specular Mask(RGB). Smoothness(A)", "You must know how \"specular setup\" works. (RGB) intensity means more metallic, lower color saturation means more metallic (reflects without tint from surface). (A) is Smoothness value."), _HighColor_Tex, _SpecColor);
            ...
            if (showToonramp)
            {
                ...
                materialEditor.TexturePropertySingleLine(new GUIContent("Main Texture (Ramp Forward Source)", "Main texture. As Forward Area intended for surface most towards light and the visual effect of being in direct light."), _MainTex);
                EditorGUI.indentLevel++;
                ...
                if (showExtraAlbedoTextures)
                {
                    ...
                    showToonRampHSVI = ACLStyles.ShurikenFoldout("Area Color Mods:", showToonRampHSVI, EditorGUI.indentLevel);
                    if (showToonRampHSVI)
                    {
                        EditorGUI.indentLevel++;
                ...
                materialEditor.ShaderProperty(_BaseColor_Step, new GUIContent("Step", "Were Forward Area blends to Core and Core overwraps Backwards Area\n0.5 is perpendicular to direct light."));
                EditorGUI.indentLevel++;
                ...
                materialEditor.ShaderProperty(_ShadeColor_Step, new GUIContent("Step", "Were Backward Area blends behind & within Core Area.\n0.5 is perpendicular to direct light."));
                EditorGUI.indentLevel++;
                ...
                showLightMapShiftMasks = ACLStyles.ShurikenFoldout("Toon Ramp Shift Masks", showLightMapShiftMasks, EditorGUI.indentLevel);
                if (showLightMapShiftMasks)
                {
                    ...
                    materialEditor.TexturePropertySingleLine(new GUIContent("Core AO (G)", "Manually forces diffuse \"Forwards\' Area to \'Core\' Area. Use to manually blend toon shadows by texture UV dynamically and union to light angle... which typically if painted on looks \"baked\" or unrealistic.\nYou may use a Ambient Occlusion Texture for this."), _Set_1st_ShadePosition);
                    materialEditor.TexturePropertySingleLine(new GUIContent("Backward AO (G)", "Manually forces diffuse \"Core\' Area to \'Backwards\' Area. Use to manually blend toon shadows by texture UV dynamically and union to light angle... which typically if painted on looks \"baked\" or unrealistic.\nYou may use a Ambient Occlusion Texture for this."), _Set_2nd_ShadePosition);
                    EditorGUILayout.LabelField("");
                ...
                if (showToonLightBehaviours)
                {
                    ...
                    materialEditor.ShaderProperty(_ToonRampLightSourceType_Backwards, new GUIContent("Backface Light Mode", "Sets light model on the back facing area to be indirect light and how much. For pbr/npr effects on diffuse backface area.\nAll Light: Adds direct and indirect light together.\nNatural ambient: Closer to PBR, backface is only Indirect light as there is realistically no direct light."));
                    EditorGUI.indentLevel++;
            ...
            if (showEmission)
            {
                ...
                materialEditor.ShaderProperty(_emissionProportionalLum, new GUIContent("Adaptive Lum", "For Unrealistic proportional glow to world brightness. Scales color to the average lighting. You might want intensity higher than Emission Color's intensity."));
                materialEditor.TexturePropertySingleLine(new GUIContent("Color Tint (RGB)", "Source glow color."), _Emissive_Tex, _uvSet_EmissionColorTex);
                EditorGUI.indentLevel++;
                ...
                EditorGUI.indentLevel++;
                materialEditor.TexturePropertySingleLine(new GUIContent("Area mask (G)", "A stronger override mask. If set bright areas will only glow. Can use this for pairing with Color Tint textures."), _EmissionColorTex);
                EditorGUI.indentLevel++;
